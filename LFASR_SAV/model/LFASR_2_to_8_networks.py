...
"""
Networks for LFASR from 2*2 to 8*8 views.
"""
    ...
    def __init__(self, block_num, block_mode="SAS", fn=64, init_gain=1.0):
        ...
        if block_mode == "SAS":
            ...
            alt_blocks = [SAV_parallel(SAS_para=sas_para, SAC_para=sac_para, feature_concat=False) for _ in
                          range(block_num)]
        elif block_mode == "SA_all_parallel":
            alt_blocks = [SA_all_parallel(act='lrelu', fn=fn) for _ in range(block_num)]
        elif block_mode == "SAV_double_serial":
            alt_blocks = [SAV_double_serial(SAS_para=sas_para, SAC_para=sac_para) for _ in range(block_num)]
        else:
