...
if opt.scale == 2:
    opt.model_dir = './checkpoints/LFSSR_SAV_x2_BasicLFSR/model_ckpt.pth'
    opt.layer_num = 16
    opt.angular_num = 5
    opt.fn = 45
    opt.mode = 'parares'
else:
    opt.model_dir = './checkpoints/LFSSR_SAV_x4_BasicLFSR/model_ckpt.pth'
    opt.layer_num = 10
    opt.angular_num = 5
    opt.fn = 45
    opt.mode = 'parares'
...
data_path = join("./TestData/BasicLFSR/test_BasicLFSR_x{}.h5".format(opt.scale))
test_set = DatasetFromHdf5_withScale_test(data_path, opt.scale)
test_loader = DataLoader(dataset=test_set, batch_size=1, shuffle=False)

test_list_file = './TestData/BasicLFSR/BasicLFSR_test_list.txt'
...
def test(name_list):
    ...
    with torch.no_grad():
        for k, batch in enumerate(test_loader):
            ...
            lfname = name_list[k]
            dataset_name = lfname.split('_')[0]
            if not dataset_name == 'EPFL':
                dataset_name += '_' + lfname.split('_')[1]

            save_dir_mat = './LFSSR_results/BasicLFSR_5x5/{}xmat/{}'.format(opt.scale, dataset_name)
            if not os.path.exists(save_dir_mat):
                os.makedirs(save_dir_mat)

            lfname = lfname[len(dataset_name) + 1:]
            print('testing dataset {}: LF {}'.format(dataset_name, lfname))
    ...
    print('Over all {} LFIs on all sets: Avg. Y PSNR: {:.2f}'.format(len(test_loader), np.mean(lf_psnr_y_list)))
